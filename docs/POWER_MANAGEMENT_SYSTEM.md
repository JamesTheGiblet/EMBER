...
Simple voltage divider wiring:

Battery + goes to one leg of the 20kΩ resistor.
The other leg of the 20kΩ resistor connects to one leg of the 10kΩ resistor and also to the wire going to ESP32 ADC pin (GPIO39).
The other leg of the 10kΩ resistor goes to battery - (GND).

So:

Battery + → 20kΩ Resistor → (join) → 10kΩ Resistor → Battery - (GND)
                           │
                           └→ ESP32 ADC pin (GPIO39)
